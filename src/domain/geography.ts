...
const MAX_DISTANCE_IN_FRANCE = 1_800_000;
...
export function computeProximityPercent(distance: number): number {
  const proximity = Math.max(MAX_DISTANCE_IN_FRANCE - distance, 0);
  return Math.floor((proximity / MAX_DISTANCE_IN_FRANCE) * 100);
}
